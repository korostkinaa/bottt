...
bot = telebot.TeleBot('5564454053:AAGbprlQJgGfLakRgOpPfSYGuUoHX9aOVW4')  # Создаем экземпляр бота
...
def get_text_messages(message):
    ...
    if subMenu is not None:
        ...
        if subMenu.name == "Игра в 21":
            ...
            # start_game = game_s.newGame(chat_id, game_s.start_game())
    ...
    if cur_menu is not None and ms_text in cur_menu.buttons:  # проверим, что команда относится к текущему меню
        ...
        elif ms_text == "Своя игра":
            game_s.start_game(bot, chat_id)
...
def callback_worker(call):
    ...
    if menu == "GameRPSm":
        ...
        game_s.callback_worker(bot, cur_user, cmd, par, call)


# -----------------------------------------------------------------------
def send_help(bot, chat_id):
    bot.send_message(chat_id, "Авторы: Коросткина и Фальшунова")
